...
import { useGetBOEOverview } from "../useBOE.tsx/useBOEApi";

const useP1ContainerTable = (id: string) => {
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => {
    const headers = ["SB No", "S No", "Container No", "Seal No", "Date"];

    const dynamicColumns = headers.map((header) => ({
      header,
      accessorKey: header, // ✅ IMPORTANT (no conversion)
      cell: (info: any) => {
...
export default useP1ContainerTable;
