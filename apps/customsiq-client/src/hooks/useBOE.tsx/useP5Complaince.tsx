...
const useComplaince = (id: string) => {
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => {
    return [
      ...
      {
        header: "Compulsory Compliance",
        accessorKey: "Compulsory Compliance",
        cell: (i: any) => (
          <div className="max-w-5xl truncate">   <LongTextCell value={i.getValue()} /></div>
        ),
      },
    ];
  ...
  return {
    table: useReactTable({
      ...
      initialState: {
        columnOrder: ["select-col", "be_no", "Compulsory Compliance"],
      },
      getRowId: (row) => row.id,
