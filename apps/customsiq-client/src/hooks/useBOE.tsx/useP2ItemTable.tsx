...
const userP2ItemTable = (id: string) => {
  ...
  const { data, isPending, isError } = useGetBOEOverview(id, "boe", "P2 Item Details", undefined, page, limit);
  ...
  const columns = useMemo<ColumnDef<any>[]>(() => {
    return [
      ...
      },
      {
        ...
        cell: (i: any) => (
          <p className="max-w-75 truncate">{i.getValue() || "-"}</p>
        ),
      },
      {
        header: "Item Unit Price",
        accessorKey: "Item Unit Price",
        cell: (i: any) => <p>{i.getValue() || "-"}</p>,
      },
      {
...
export default userP2ItemTable;
