...
import {
  ...
  useDownloadItemDetails,
  useItemDetailsFilter,
} from "@/src/hooks/useReports";

import SearchBar from "@/src/ui/Searchbar";
import DateFilter from "../DateFilter";
import FilterPanel from "./filters/FilterPannel";
import useItemDetailsTable from "@/src/hooks/useItemDetails/useItemDetailsTable";
...
const ItemDetail = ({ report_name }: { report_name: string }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const userData = useUserStore((state) => state.user);

  const { mutateAsync, isPending } = useDownloadItemDetails();
  const params = Object.fromEntries(searchParams.entries());
  const { data: filter } = useItemDetailsFilter(userData?.id || "");
  ...
  const {
    ...
    page,
    handlePage,
  } = useItemDetailsTable(userData?.id || "");
