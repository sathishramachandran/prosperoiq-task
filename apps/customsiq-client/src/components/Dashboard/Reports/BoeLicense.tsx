...
import useBoeLicenseTable from "@/src/hooks/useBoeLicense";
import {
  useBOELicenseFilter,
...
const BoeLicense = ({ user_id }: { user_id: string }) => {
  ...
  const { data: filter } = useBOELicenseFilter(userData?.id || "");
  ...
  const {
    ...
    page,
    handlePage,
  } = useBoeLicenseTable(user_id, "json");
