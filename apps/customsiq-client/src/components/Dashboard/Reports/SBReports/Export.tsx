...
import useExportTable from "@/src/hooks/useSBReports/useExport";
...
const Export = ({ user_id, download_func }: { user_id: string; download_func: () => Promise<void> }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { data: filter } = useExportReportFilter(user_id || "");
  const {
    isLoading,
    dashboardvalue,
    isError,
      ...
      <div className="flex mt-4">
        ...
        <AnimatePresence>
          {showFilter && (
            ...
            >
              <div className="overflow-auto h-[calc(100vh-240px)]  rounded-sm py-3 px-4 bg-white w-65">
                {filter && (
                  <FilterPanel
                    value={filters}
