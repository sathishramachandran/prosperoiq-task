...
const InvoiceDetail = ({ user_id, download_func }: { user_id: string; download_func: () => Promise<void> }) => {
  ...
  const {
    ...
    handlePage,
    dashboardvalue,
  } = useInvoiceTable(user_id, "json");

  const { data: filter } = useSBIFilter(user_id || "", "p4-invoice-details");
  ...
  const formatValue = (key: string, value: any) => {
    ...
    if (key.includes("Days")) return Math.round(value);
      ...
      <div className="flex gap-4 mb-4">
        {dashboardvalue &&
          Object.entries(dashboardvalue).map(([key, value], index) => {
            // ✅ Top Destination — unique shape { destination, total_invoice_amount }
            if (key === "Top Destination") {
              return (
                <div
                  key={key}
                  ...
                  {value ? (
                    <div className="mt-2 space-y-1">
                      <p className="text-sm font-semibold">
                        {(value as any).destination}
                      </p>
                      <p className="text-sm text-gray-600">
                        {formatINR((value as any).total_invoice_amount)} (EUR)
                      </p>
                    </div>
                  ) : (
...
function isDashboardEntry(v: unknown): v is DashboardEntry {
  ...
  if ("display" in obj || "destination" in obj) return false;
  return (
    "value" in obj &&
