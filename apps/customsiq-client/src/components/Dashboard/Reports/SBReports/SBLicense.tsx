...
import useLicenseTable from "@/src/hooks/useSBReports/useLicense";
...
const SBLicense = ({ user_id, download_func }: { user_id: string; download_func: () => Promise<void> }) => {
  ...
  const {
    ...
    page,
    handlePage,
  } = useLicenseTable(user_id, "json");
  const { data: filter } = useSBIFilter(user_id || "", "aa-license");
  ...
  const formatValue = (val: any) => {
    // Convert string numbers like "99876593.2 "
