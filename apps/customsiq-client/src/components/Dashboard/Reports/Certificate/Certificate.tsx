...
import userCertificateTable from "@/src/hooks/useCertificate.tsx/useCertificateTable";
import { useBOEReports } from "@/src/hooks/useBOE.tsx/useBOEApi";
import { useRouter, useSearchParams } from "next/navigation";
import {
  useBOECertificateFilter,
...
const Certificate = ({ report_name }: { report_name: string }) => {
  ...
  const { data: filter } = useBOECertificateFilter(userData?.id || "");
  const searchParams = useSearchParams();
  const params = Object.fromEntries(searchParams.entries());

  const handleFiltersChange = (val: Record<string, string[]>) => {
    setLocalFilters(val); // for UI (chips)
    setFilters(val); // for API
  };
  ...
  const {
    ...
    page,
    handlePage,
  } = userCertificateTable(userData?.id || "", report_name);
