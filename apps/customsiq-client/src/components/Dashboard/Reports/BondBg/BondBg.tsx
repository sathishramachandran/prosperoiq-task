...
import userBONDTable from "@/src/hooks/useBondBG/useBondBG";
import {
  useBONDBGReportFilter,
  useDownloadBondBgMainReport,
...
const BondBg = ({ report_name }: { report_name: string }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const userData = useUserStore((state) => state.user);

  const { mutateAsync, isPending } = useDownloadBondBgMainReport();
  const params = Object.fromEntries(searchParams.entries());
  const { data: filter } = useBONDBGReportFilter(userData?.id || "");
  ...
  // console.log(isError, "isErrorisErrorisErrori/sError");

  const [showFilter, setShowFilter] = useState(false);

  const handleReports = async () => {
    await mutateAsync({ user_id: userData?.id || "", filter: params });
  };
            ...
            return (
              ...
              >
                ...
                <p className="text-sm text-slate-900 font-semibold mt-2">
                  {key === "Total Amount Utilized"
                    ? `${num.toLocaleString("en-IN")} INR`
      ...
      <div className="flex mt-4">
        ...
        <AnimatePresence>
          {showFilter && (
            ...
            >
              <div className="overflow-auto h-[calc(100vh-225px)] rounded-sm py-3 px-4 bg-white w-65">
                {filter && (
                  <FilterPanel
                    value={filters}
        ...
        <motion.div className="flex-1 overflow-auto ">
          ...
          {!isError && !isLoading && !!data.length && (
            <div className="overflow-x-auto h-[calc(100vh-285px)] ">
