...
import useP4SingleTable from "@/src/hooks/useBOE.tsx/useP4Single";
import Loading from "@/src/ui/Loading";
import { TableContainer } from "prosperoiq-table";

const P4Single = ({ job_id }: { job_id: string }) => {
  const { isLoading, isError, data, table, total_pages, page, handlePage, total_data } = useP4SingleTable(job_id);
  return (
    <div>
      {isLoading && <Loading />}
