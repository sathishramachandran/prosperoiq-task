...
import userP2ItemDutyTable from "@/src/hooks/useBOE.tsx/useP2ItemDuty";
import Loading from "@/src/ui/Loading";
import { TableContainer } from "prosperoiq-table";

const P1ItemDuty = ({ job_id }: { job_id: string }) => {
  const { isLoading, isError, data, table, total_pages, page, handlePage, total_data } = userP2ItemDutyTable(job_id);
  return (
    <div>
      {isLoading && <Loading />}
