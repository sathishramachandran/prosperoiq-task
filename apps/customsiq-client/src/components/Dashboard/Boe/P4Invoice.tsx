...
import useP4Invoice from "@/src/hooks/useBOE.tsx/useP4Invoice";
import Loading from "@/src/ui/Loading";
import { TableContainer } from "prosperoiq-table";

const P4Invoice = ({ job_id }: { job_id: string }) => {
  const { isLoading, isError, data, table, total_pages, page, handlePage, total_data } = useP4Invoice(job_id);
  return (
    <div>
      {isLoading && <Loading />}
